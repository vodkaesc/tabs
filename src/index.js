...
chrome.storage.sync.get(
  ...
  (result) => {
    ...
    if (result.searchBorderColor)
      root.style.setProperty("--search-focus-color", result.searchBorderColor);
    if (result.searchTextColor)
      root.style.setProperty("--search-text-color", result.searchTextColor);
    if (result.searchIconColor)
      root.style.setProperty("--search-icon-color", result.searchIconColor);
    if (result.searchPlaceholder)
      searchInput.placeholder = result.searchPlaceholder;
    ...
    bookmarks.forEach((bm, i) => {
      ...
      if (animEnabled) {
        ...
        else if (bookmarkTiming === "right")
          delay = 0.2 + (bookmarks.length - 1 - i) * 0.06;
    ...
    if (result.messageEnabled === false) {
      ...
      if (messageType === "afternoon-morning")
        ...
        greeting = now.toLocaleDateString([], {
          weekday: "long",
          month: "long",
          day: "numeric",
        });
      else if (messageType === "time-12")
        greeting = now.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
          hour12: true,
        });
